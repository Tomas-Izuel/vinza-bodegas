...
interface BodegaDetalleEditFormProps {
  bodega: BodegaDetalleType;
  onBodegaUpdated?: (bodegaActualizada: BodegaDetalleType) => void;
  onCancel: () => void;
}

export function BodegaDetalleEditForm({
  bodega,
  onBodegaUpdated,
  onCancel,
}: BodegaDetalleEditFormProps) {
  ...
  const [existingMultimedia, setExistingMultimedia] = useState<Multimedia[]>(
    bodega.multimedia || [],
  );
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  const [deletedMultimediaIds, setDeletedMultimediaIds] = useState<number[]>(
    [],
  );
  ...
  const removeNewFile = useCallback(
      ...
      setSelectedFiles((prev) => {
        ...
        if (fileToRemove.name === multimediaPortada) {
          const firstExisting = existingMultimedia.find(
            (m) => !deletedMultimediaIds.includes(m.id),
          );
    ...
    [multimediaPortada, existingMultimedia, deletedMultimediaIds],
  );
  ...
  const removeExistingMultimedia = useCallback(
    (multimediaId: number) => {
      ...
      setDeletedMultimediaIds((prev) => [...prev, multimediaId]);
    },
    [deletedMultimediaIds, selectedFiles],
  );
  ...
  const onSubmit = async (data: EditarBodegaType) => {
    try {
      ...
      if (deletedMultimediaIds.length > 0) {
        formData.append(
          "deleteMultimedia",
          JSON.stringify(deletedMultimediaIds),
        );
      }
      ...
      const bodegaActualizada = await actualizarBodega(formData);
      onBodegaUpdated?.(bodegaActualizada);

      // Actualizar estado local con la respuesta
      setExistingMultimedia(bodegaActualizada.multimedia || []);
      setSelectedFiles([]);
  ...
  const cancelarEdicion = () => {
    ...
    setSelectedFiles([]);
    setDeletedMultimediaIds([]);
    const portada = bodega.multimedia.find((m) => m.es_portada);
  ...
  const hasChanges =
    ...
    deletedMultimediaIds.length > 0 ||
    (() => {
      const currentPortada = existingMultimedia.find((m) => m.es_portada);
                  ...
                  <div>
                    ...
                    <p
                      className={`text-base font-medium ${
                        bodega.validada === null
                          ? "text-yellow-600"
                          : "text-green-600"
                      }`}
                    >
                      {bodega.validada === null ? "Pendiente" : "Validada"}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500 mb-2">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Gestiona las imágenes de tu bodega
                      </p>
                    </div>
                  ...
                  {(existingMultimedia.filter(
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 max-h-48 overflow-y-auto">
                        ...
                        {existingMultimedia
                          .filter((m) => !deletedMultimediaIds.includes(m.id))
                          .map((multimedia) => {
